...
class RadarSensor:
    ...
    def beamform_tx_direction(self, rd_cube_txrx, theta_deg, phase_sign=-1):
        ...
        # rd_cube_txrx is [range, tx, rx, doppler]
        beam_cube_rx = np.einsum("a b c d, b d -> a c d", rd_cube_txrx, tx_combiner)
...
def nms_bev_detections(det_list, range_merge_m=0.35, lateral_merge_m=0.35, vel_merge_mps=1.0):
    """
    det_list entries should contain:
      {
        "r_idx", "d_idx",
        "range_m", "vel_mps", "angle_deg",
        "x_m", "y_m",
        "power"
      }

    Greedy NMS:
    - sort by strongest power first
    - keep strongest
    - suppress nearby detections in BEV/range-velocity neighborhood
    """
    if not det_list:
        return []
    ...
    for cand in det_sorted:
        ...
        for kept_det in kept:
            dx = cand["x_m"] - kept_det["x_m"]
            dy = cand["y_m"] - kept_det["y_m"]
            dv = cand["vel_mps"] - kept_det["vel_mps"]

            lateral_close = abs(dx) <= lateral_merge_m
            range_close = abs(dy) <= range_merge_m
            vel_close = abs(dv) <= vel_merge_mps

            if lateral_close and range_close and vel_close:
                suppress = True
                break
...
def run_radar_simulation():
    ...
    log_file_path = os.path.join(log_dir, "radar_stream_log_multibeam_nms.txt")
    ...
    with open(log_file_path, "w", encoding="utf-8") as f:
        for frame_idx in range(ss.num_frames):
            ...
            ax_rd.set_title(f"Best-Beam RD | Frame {frame_idx}")
            ...
            # Build raw detection list before NMS
            det_candidates = []
            ...
            for r_idx, d_idx in detections:
                ...
                beam_idx = int(best_beam_idx[r_idx, d_idx])
                coarse_beam_angle_deg = float(beam_angles_deg[beam_idx])

                if use_rx_fft_for_fine_aoa:
                    winning_beam_cube = beam_cube_list[beam_idx]
                    aoa_deg = estimate_rx_aoa_deg(radar, winning_beam_cube, r_idx, d_idx)
                else:
                    aoa_deg = coarse_beam_angle_deg
                ...
                det_candidates.append({
                    ...
                    "beam_deg": float(coarse_beam_angle_deg),
                    "x_m": float(x),
                    "y_m": float(y),
                    "power": float(best_rd_power[r_idx, d_idx]),
                })

            # Apply cross-beam / BEV NMS
            det_final = nms_bev_detections(
                det_candidates,
                range_merge_m=range_merge_m,
                lateral_merge_m=lateral_merge_m,
                vel_merge_mps=vel_merge_mps,
            )
